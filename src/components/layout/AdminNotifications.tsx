...
export function AdminNotifications() {
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      ...
      const outOfStock = variants?.filter(v => v.stock_quantity === 0).length || 0
      const lowStock = variants?.filter(v => v.stock_quantity > 0 && v.stock_quantity <= 5).length || 0
            ...
            >
              ...
              <div className="p-4 space-y-2">
                ...
                <Link 
                  ...
                  className="flex items-center gap-4 p-4 rounded-2xl hover:bg-muted/50 transition-all"
                >
                  <div className={`p-2.5 rounded-xl ${alerts.pendingOrders > 0 ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground'}`}>
